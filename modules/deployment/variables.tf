...
variable "internal_port" {
  type        = list(object({ name = optional(string), internal_port = number, host_port = optional(number) }))
  description = "(Optional) List of ports to expose from the container"
  default     = []
}
variable "volume_mount" {
  type        = list(object({ mount_path = string, sub_path = optional(string), volume_name = string, read_only = optional(bool) }))
